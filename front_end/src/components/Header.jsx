...
const Header = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Transition to full width precisely when entering the Core Capabilities section
      // (which is right after the 100vh Hero section). We use a small offset so it happens just as it enters.
  ...
  return (
    <>
      <header className={`fixed top-0 left-0 w-full z-50 flex justify-center pointer-events-none transition-all duration-700 ease-in-out ${isScrolled ? "pt-0" : "pt-6"}`}>
        <div
          className={`pointer-events-auto flex items-center justify-between transition-all duration-700 ease-in-out overflow-hidden ${isScrolled
            ? "w-full max-w-[1920px] bg-white/80 backdrop-blur-xl border-b border-white/20 shadow-sm rounded-none px-4 md:px-6 py-3"
            : "w-[95%] md:w-[90%] max-w-[1200px] bg-[#E8E6DF]/80 backdrop-blur-xl border border-white/40 shadow-[0_8px_32px_rgba(0,0,0,0.15)] rounded-full px-4 md:px-8 py-2"
            }`}
        >
          {/* Logo */}
          <div className="flex items-center justify-center w-24 md:w-32 relative h-10 md:h-14 shrink-0">
            <img
              src={logoShield}
              alt="JOVA METCRAFT"
          ...
          {/* Desktop Navigation */}
          <nav className="hidden xl:flex items-center space-x-8 text-[13px] font-bold text-gray-800 tracking-wider">
            ...
            <Link to="/Product" className="hover:text-[#FF6B00] transition-colors">Product</Link>
            <Link to="/services" className="hover:text-[#FF6B00] transition-colors">Services</Link>
      ...
      >
        ...
        >
          ...
          <nav className="flex flex-col space-y-6 text-lg font-bold text-gray-800 tracking-wide">
            <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-[#FF6B00] transition-colors pb-4 border-b border-gray-100">Home</Link>
            <Link to="/about" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-[#FF6B00] transition-colors pb-4 border-b border-gray-100">About</Link>
            <Link to="/Product" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-[#FF6B00] transition-colors pb-4 border-b border-gray-100">Product</Link>
            <Link to="/services" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-[#FF6B00] transition-colors pb-4 border-b border-gray-100">Services</Link>
            <a href="/#contact" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-[#FF6B00] transition-colors pb-4 border-b border-gray-100">Contact</a>
          </nav>
